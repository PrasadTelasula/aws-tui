...
// ─── Shared info popup renderer ──────────────────────────────────────────────

pub(super) fn colorize_json_line(line: &str) -> Line<'static> {
    ...
    if let Some(colon_pos) = trimmed.find("\": ") {
        ...
        let key_part = &trimmed[..key_end + 1];
        let rest = &trimmed[key_end + 2..];
...
/// Generic scrollable info popup — reused by both the Instances and Containers tabs.
/// `popup` — the popup data; `border_color` — accent color for the border.
pub(super) fn draw_info_popup(
    f: &mut Frame,
    area: Rect,
    popup: &crate::instances::InstanceInfoPopup,
    border_color: Color,
    spinner: &str,
) {
    ...
    let title_hint = Span::styled("   Tab: switch  j/k: scroll  /: search  q: close", Style::default().fg(FG4));
    ...
    if popup.loading {
        f.render_widget(
            Paragraph::new(Span::styled(
                format!("  {} Fetching details…", spinner),
    ...
    let lines: Vec<Line> = lines_data
        ...
        .map(|(abs_idx, l)| {
            let line = if l.starts_with("──") {
                Line::from(Span::styled(l.clone(), Style::default().fg(border_color).add_modifier(Modifier::BOLD)))
            } else if l.starts_with("  Error") {
                Line::from(Span::styled(l.clone(), Style::default().fg(RED)))
            } else if popup.tab == InfoTab::Json {
                colorize_json_line(l)
            } else if l.starts_with("  ") && l.len() > 2 {
                let trimmed = &l[2..];
                if let Some(gap) = trimmed.find("  ") {
                    ...
                    Line::from(vec![
                        Span::raw("  "),
                        Span::styled(format!("{:<22}", label), Style::default().fg(FG2)),
                        Span::styled(value.to_string(), Style::default().fg(FG)),
                    ])
                } else {
                    Line::from(Span::styled(l.clone(), Style::default().fg(FG2)))
                }
            } else {
                Line::from(Span::styled(l.clone(), Style::default().fg(FG2)))
            };
    ...
    if let Some(sb_area) = search_bar_area {
        ...
        let bar_spans = if popup.search_active {
            vec![
                ...
                Span::styled(popup.search_query.clone(), Style::default().fg(FG)),
            ...
            vec![
                ...
                Span::styled(popup.search_query.clone(), Style::default().fg(TEAL)),
                Span::styled(match_info, Style::default().fg(TEAL)),
                Span::styled("  Esc to clear", Style::default().fg(FG4)),
            ]
        };
        f.render_widget(Paragraph::new(Line::from(bar_spans)), sb_area);
    }
}
